...
export const mySchema = new Schema({
  nodes: {
    ...
    list_item: {
      ...
      parseDOM: [{tag: "li"}],
      toDOM(node) { return ["li", {class: node.attrs.task ? "task-list-item": "" }, 0] }
    },

    check_box: {
      attrs: {checked: {default: false}},
      group: "block",
      parseDOM: [{tag: "input[type=checkbox]", getAttrs: checkbox => ({checked: (checkbox as HTMLInputElement).checked})}],
      toDOM(node) { return ["input", {"class": "task-list-item-checkbox", "type": "checkbox", "checked": node.attrs.checked} ]}
    },
