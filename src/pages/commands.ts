...
export const listNewline: Command = (state, dispatch) => {
  let {$head, $anchor} = state.selection
  let types: (null | {
    type: NodeType;
    attrs?: Attrs | null;
  ...
  for (let d = $head.depth; d > 0; d--) {
    let node = $head.node(d);
    types.unshift(node);
  }
  console.log(types);
  if (dispatch) dispatch(state.tr.split($head.pos, types.length, types).scrollIntoView())
...
export const pcBaseKeymap: {[key: string]: Command} = {
  "Enter": chainCommands(liftEmptyBlock, listNewline),
  "Mod-Enter": exitCode,
  "Backspace": backspace,
